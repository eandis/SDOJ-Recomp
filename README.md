...
The slowdown is the same as Xbox 360, not including the hardware lag. That means the game can not go lower than half speed, and some places will have slightly different slowdown. Tuning slowdown is the main prirority right now. The regular non-expert modes are currently tuned to pretty closely match what you would see on shmupmame 5.3.1, expert mode is still being tuned. Xbox 360 Arrange does **not** have any slowdown, as all the slowdown comes from the 360 lagging itself, thus, the slowdown will not be tuned for this mode.
